...
describe('Llenar los campos para una nueva cita y editarlo', () => {
    ...
    it('Editar el paciente', () => {
        cy.get('[data-cy="btn-editar"]').click();
        cy.get('[name="mascota"]').clear().type('Sombra bebé');

        cy.get('[data-cy="submit-citas"]').click();

         // SEleccionar la alerta
         cy.get('[data-cy="alerta"]').invoke('text').should('equals', 'Guardado Correctamente');
